...
char * keys[] = {"just", "a", "few", "keys", "for", "testing", "and", "another", "some", "more", "what's the duplicate?", "what's the story duplicate?"};
...
int main(int argv, char * argc[]) {
	...
	for (i = 0; i < (sizeof(keys) / sizeof(char *)); i++) {
		printf("Set %s => %s\n", keys[i], values[i]);
		hashtable_set(&test, keys[i], strlen(keys[i]), values[i]);
	}
